...
import { Boxes, CheckCircle2, PackageX, AlertTriangle, DollarSign, ArrowUp, ArrowDown, type LucideIcon } from "lucide-react";
import { Card } from "@/components/ui/Card";
import { productsKpis, type ProductsKpi } from "@/lib/products-data";

const iconMap: Record<ProductsKpi["icon"], LucideIcon> = {
  boxes: Boxes,
  check: CheckCircle2,
  "package-x": PackageX,
...
export default function ProductsKpiRow() {
      ...
      {productsKpis.map((kpi) => {
        ...
        return (
          <Card key={kpi.id} className="p-4 flex flex-col gap-2.5 relative">
            <button onClick={() => setOpenId(open ? null : kpi.id)} className="flex items-start justify-between text-left w-full">
