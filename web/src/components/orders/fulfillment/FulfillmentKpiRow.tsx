...
import { Clipboard, Lock, Package, Truck, CircleCheck, TriangleAlert, type LucideIcon } from "lucide-react";
import { Card } from "@/components/ui/Card";
import { fulfillmentKpis, type FulfillmentKpi } from "@/lib/fulfillment-data";

const iconMap: Record<FulfillmentKpi["icon"], LucideIcon> = {
  clipboard: Clipboard,
  lock: Lock,
  package: Package,
  truck: Truck,
...
export default function FulfillmentKpiRow() {
  const [openId, setOpenId] = useState<string | null>(null);

  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4">
      {fulfillmentKpis.map((kpi) => {
