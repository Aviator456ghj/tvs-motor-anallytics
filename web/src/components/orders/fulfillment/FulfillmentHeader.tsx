    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <div>
        <div className="flex items-center gap-1.5 text-[12.5px] text-muted-light mb-1">
          ...
          <span>Orders</span>
          <ChevronRight size={13} />
          <span className="text-foreground font-medium">Fulfillment Center</span>
        </div>
        <h1 className="text-2xl font-bold text-foreground tracking-tight">Fulfillment Center</h1>
      </div>
          ...
          {createOpen && (
            <>
              ...
              <div className="absolute right-0 top-full mt-1 w-52 bg-card-bg border border-card-border rounded-lg shadow-lg z-20 py-1">
                {["From pending orders", "Manual fulfillment", "Split shipment"].map((opt) => (
