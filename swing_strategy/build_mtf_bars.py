"""
Build aligned 1-minute / 15-minute / 4-hour BTCUSD OHLCV bars from the raw
trade-by-trade dump already fetched for the 5-min CVD variant
    ...
    with open(RAW_CHECKPOINT) as f:
        ...
        for price, volume, t, side in reader:
            ...
            for name, (secs, _) in TIMEFRAMES.items():
                ...
                b = buckets[name].setdefault(bucket_ts, {"o": None, "h": -1.0, "l": 1e18, "c": None, "vol": 0.0})
                if b["o"] is None:
                    b["o"] = price
                b["h"] = max(b["h"], price)
                b["l"] = min(b["l"], price)
    ...
    for name, (secs, path) in TIMEFRAMES.items():
        ...
        with open(path, "w", newline="") as f:
            ...
            for ts, b in rows:
                ...
                w.writerow([ts, dt, b["o"], b["h"], b["l"], b["c"], round(b["vol"], 8)])
